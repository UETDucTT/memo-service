...
export class tagTable1609575809205 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `diary` ADD CONSTRAINT `FK_33ca9df2514f801679dd6da724d` FOREIGN KEY (`tagId`) REFERENCES `tag`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION',
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE `diary` DROP FOREIGN KEY `FK_33ca9df2514f801679dd6da724d`',
    );
    await queryRunner.query(
    ...
    await queryRunner.query('ALTER TABLE `diary` DROP COLUMN `tagId`');
    await queryRunner.query(
      'ALTER TABLE `diary` ADD `tagId` varchar(255) NULL',
    );
    await queryRunner.query('DROP TABLE `tag`');
    await queryRunner.query(
      'ALTER TABLE `diary` CHANGE `tagId` `tag` varchar(255) NULL',
    );
  }
}
